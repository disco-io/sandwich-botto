...
# ---------------------------------------------------------------------------------------
  ...
  if message.content.startswith('>addbread'):
    ...
    #input_bread = message.channel.split('$new ', 1)[1]
    #update_bread(input_bread)
    await message.channel.send(input_bread + ' has been added to the bread database!')

  if message.channel.startswith('>delbread'):
    bread = []
    if "bread" in db.keys():
      #index = int(msg.split('>delbread',1)[1])
      #delete_bread(index)
      #bread = db["bread"]
